...
RUN curl -Lks http://www.atlassian.com/software/jira/downloads/binary/atlassian-jira-software-$JIRA_VERSION-jira-$JIRA_VERSION.tar.gz -o /tmp/jira.tar.gz \
	&& mkdir -p /opt/jira \
	...
	&& sed --in-place          "s/java version/openjdk version/g" "/opt/jira/bin/check-java.sh" \
	&& rm /tmp/jira.tar.gz
